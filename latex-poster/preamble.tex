...
% Poster environment (header / footer / body scaffold)
    \NewDocumentEnvironment{poster}{ +b }{\begin{tikzpicture}[remember picture,overlay,inner sep=0pt,outer sep=0pt]
        ...
        \node[anchor=north,font=\fontsize{16pt}{18pt}\bfseries\color{COLtitle}\selectfont] at ([yshift=-4.1cm]QRL.north) {\href{\dbQRcodeB}{10.5281/zenodo.21042345}};
...
% Tighten the bibliography for the poster References box. \setlist{nosep} only
% affects itemize/enumerate, not the biblatex list -- so without this the
% References box gets a large leading gap (top white space) and runs off the
% page bottom. Kill the list's leading skip and compress inter-entry spacing.
\setlength{\bibitemsep}{2pt}
\AtBeginBibliography{\vspace*{-0.7cm}}
